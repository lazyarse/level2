...
| Inference binding | Reuse the project's established LiteRT stack via a **hand-rolled `flutter_litert` wrapper** (**`CompiledModel` — LiteRT Next**). The `yolo26n_w8a32.tflite` `format=litert` export targets the LiteRT Next runtime: the classic `Interpreter` loads it but fails at `TfLiteInterpreterInvoke` with `kTfLiteError` (verified on Linux desktop), so the engine runs through `CompiledModel.fromBuffer` → `run([Float32List])` → `List<Float32List>` (CPU accelerator; the model's float32 NCHW I/O fits CompiledModel's float32-only API). Face detection uses `flutter_litert`'s `Interpreter` (works on Android AND Linux desktop — the fast dev loop). Person detection follows the same engine abstraction so the real-model gate can run on Linux, mirroring the face phase. Deviation from the main doc's "tflite_flutter" literal wording: `tflite_flutter` is Android-only here (its Linux CMake target collides with `flutter_litert`), and `flutter_litert` IS the LiteRT runtime — noted in the plan. |
