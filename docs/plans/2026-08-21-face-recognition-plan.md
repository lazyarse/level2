...
- [ ] **Step 1:** Obtain `mobile_face_net.tflite` (pinned source, 112×112 RGB →
  192-d float), add to `app/src/main/assets`; record SHA-256 here.
- [ ] **Step 2:** `inference/FaceEmbeddingEngine.kt` following
...
- [ ] **Step 3:** Tests — settings round-trip incl. legacy JSON; store
  save/load/delete round-trip; corrupt-bin tolerance; merge math.
- [ ] **Step 4:** Verify + commit.

### Task 3: Triggers, labels, config migration

- [ ] **Step 1:** `TriggerType.faceKnown/faceUnknown`; `triggerLabel` entries.
- [ ] **Step 2:** Enable/disable migration helpers in `SettingsRepository`/
...
- [ ] **Step 3:** Tests — labels; migration from-with/without-face-config
  states; disable/re-enable round-trips; unknown-type routing untouched.
- [ ] **Step 4:** Verify + commit.

### Task 4: Detector recognition branch

- [ ] **Step 1:** `FaceDetector` gains optional recognition collaborators
...
- [ ] **Step 2:** Tests (mock engines, pixel-coordinate boxes on ≥100 px frames):
...
- [ ] **Step 1:** Service-side hook: when monitoring and an enrollment request
...
- [ ] Staging instrumentation pass on `pixel_34_aosp`
      (`run_android_integration_tests.sh <serial> all`); kill emulator/qemu.
- [ ] Release smoke if build rules changed (asset keeps).
- [ ] Tick this plan; add parity-matrix rows; note model SHA-256 in design doc.
