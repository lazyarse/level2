...
/**
 * Dog-detection trigger. Runs on color analysis frames (motion-gated by the
 * pipeline, like [io.securitycam.level1.detection.person.PersonDetector]).
 * Persistence/threshold/cooldown come from [DetectorConfig].
 *
 * Uses the shared YOLO26n model via [YoloDogEngine] — zero extra model
 * load when the person detector is also enabled.
 */
class DogDetector(
    override val config: DetectorConfig,
    engine: DogEngine? = null,
) : FrameDetector() {

    private val engine: DogEngine = engine ?: YoloDogEngine(AppContextHolder.require())
